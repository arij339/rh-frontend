...
<div class="employes fade-in">
  ...
  <div class="page-header">
    <div class="ph-left">
      <div class="ph-icon"><span [innerHTML]="ic.users | safeHtml"></span></div>
      <div>
  ...
  <div *ngIf="activeTab() === 'liste'" class="tab-content fade-in">
    ...
    <div class="stats-row">
      <div class="stat-card" *ngFor="let s of getRoleStats()"
           [class.active]="s.role === filterRole()"
           (click)="filterRole.set(s.role === filterRole() ? '' : s.role)">
    ...
    <div class="filters-row">
      ...
      <select class="filter-sel"
              [value]="filterDept()"
              (change)="filterDept.set($any($event.target).value)">
      ...
      <select class="filter-sel"
              [value]="filterSociete()"
              (change)="filterSociete.set($any($event.target).value)">
        <option value="">Toutes les sociétés</option>
        <option *ngFor="let s of getSocietes()" [value]="s">🏢 {{ s }}</option>
      </select>
    </div>
        ...
        <table>
          ...
          <tbody>
            <tr *ngFor="let e of getFiltered()" class="tr-hover">
              ...
              <td>
                <span class="societe-tag" *ngIf="e.societe">🏢 {{ e.societe }}</span>
                <span class="no-data" *ngIf="!e.societe">—</span>
              </td>
              <td>
                <div class="manager-cell" *ngIf="e.managerNom">
                  <div class="mgr-av"
                       [style.background]="getAvatarColor({nom: e.managerNom, prenom: e.managerPrenom})">
              ...
              <td>
                <span class="role-badge" [class]="'role-' + e.role?.toLowerCase()">
                  {{ getRoleLabel(e.role) }}
                </span>
              </td>
              <td>
            ...
            <tr *ngIf="getFiltered().length === 0">
              <td colspan="8" class="empty-row">Aucun employé trouvé</td>
            </tr>
          </tbody>
        </table>
      </div>
        ...
        <div class="form-card">
          ...
          <form [formGroup]="createForm" (ngSubmit)="onCreate()">
            ...
            <div class="grid-2">
              ...
              <div class="form-group">
                <label>Email *</label>
                <input type="email" formControlName="email" placeholder="email@entreprise.tn"
            ...
            <div class="grid-2">
              ...
              <!-- ✅ SOCIÉTÉ -->
              <div class="form-group">
                <label>Société <span class="hint">optionnel</span></label>
                <input type="text" formControlName="societe"
                       placeholder="Filiale Tunis, Siège Social..." />
              </div>
              <div class="form-group">
                <label>Type de contrat *</label>
                <select formControlName="typeContrat" [class.error]="isInvalid('typeContrat')">
                  <option value="">— Choisir —</option>
                  <option value="CDI">CDI</option>
                  <option value="CDD">CDD</option>
                  <option value="STAGE">Stage</option>
                </select>
                <span class="err" *ngIf="isInvalid('typeContrat')">Obligatoire</span>
              </div>
              <div class="form-group">
              ...
              <div class="form-group">
                ...
                <input type="date" formControlName="dateDebutContrat" />
              </div>
              <div class="form-group">
              ...
              <div class="form-group">
                <label>Durée d'essai (mois)</label>
                <input type="number" formControlName="dureeEssaiMois" min="0" />
              </div>
              <div class="form-group">
            ...
            <div class="form-group manager-field" *ngIf="showManagerSelect()">
              <div class="manager-list-select">
                <div class="mls-option"
                     [class.selected]="!createForm.get('managerId')?.value"
                     (click)="createForm.get('managerId')?.setValue(null)">
                ...
                <div class="mls-option" *ngFor="let m of getManagersList()"
                  ...
                  <div class="mls-info">
                    ...
                    <small>{{ m.poste }} — {{ m.departement }}
                      <span class="role-badge sm" [class]="'role-' + m.role?.toLowerCase()">
                        {{ getRoleLabel(m.role) }}
                      </span>
                    </small>
                  </div>
            ...
            <div class="form-actions">
              ...
              <button type="submit" class="btn btn-primary" [disabled]="createLoading()">
                <span *ngIf="!createLoading()">
                  <span [innerHTML]="ic.plus | safeHtml"></span> Créer le compte
                </span>
    ...
    <div class="modifier-layout">
      ...
      <div class="emp-hero">
        ...
        <div class="eh-info">
          ...
          <span class="role-badge" [class]="'role-' + selectedEmploye()!.role?.toLowerCase()">
            {{ getRoleLabel(selectedEmploye()!.role) }}
          </span>
        </div>
      ...
      <form [formGroup]="editForm" (ngSubmit)="onSaveEdit()">
        ...
        <div class="edit-card">
          ...
          <div class="grid-3">
            <div class="form-group">
              <label>Poste</label>
              <input type="text" formControlName="poste" placeholder="Poste actuel" />
            </div>
            <div class="form-group">
              <label>Département</label>
              <input type="text" formControlName="departement" placeholder="Département" />
            </div>
            <!-- ✅ SOCIÉTÉ -->
            <div class="form-group">
              <label>Société</label>
            ...
            <div class="form-group">
              ...
              <input type="number" formControlName="salaireBase" placeholder="0.000" step="0.001" />
            </div>
            <div class="form-group">
          ...
          <div class="form-group" style="margin-top: 16px">
            ...
            <div class="manager-list-select">
              <div class="mls-option"
                   [class.selected]="!editForm.get('managerId')?.value"
                   (click)="editForm.get('managerId')?.setValue(null)">
        ...
        <div class="edit-card">
          ...
          <div class="grid-3">
            ...
            <div class="form-group">
              ...
              <input type="text" formControlName="nationalite" placeholder="Nationalité" />
            </div>
            <div class="form-group">
        ...
        <div class="edit-card">
          ...
          <div class="grid-3">
            <div class="form-group">
              <label>Nom</label>
              <input type="text" formControlName="contactUrgenceNom" placeholder="Nom du contact" />
            </div>
            <div class="form-group">
              <label>Téléphone</label>
              <input type="tel" formControlName="contactUrgenceTelephone" placeholder="Téléphone" />
            </div>
            <div class="form-group">
              <label>Relation</label>
              <select formControlName="contactUrgenceRelation">
                <option value="">— Sélectionner —</option>
                <option>Père</option>
                <option>Mère</option>
        ...
        <div class="form-alert error" *ngIf="editError()">
          <span [innerHTML]="ic.alertCircle | safeHtml"></span> {{ editError() }}
        </div>
        <div class="form-alert success" *ngIf="editSuccess()">
          <span [innerHTML]="ic.checkCircle | safeHtml"></span> {{ editSuccess() }}
        </div>
        ...
        <div class="form-actions">
          ...
          <button type="submit" class="btn btn-primary" [disabled]="editLoading()">
            <span *ngIf="!editLoading()">
              <span [innerHTML]="ic.save | safeHtml"></span> Enregistrer les modifications
            </span>
  ...
  styles: [`
    ...
    .ph-icon { width: 52px; height: 52px; background: linear-gradient(135deg, var(--c-teal), var(--c-teal-dk)); border-radius: 14px; display: flex; align-items: center; justify-content: center; color: white; flex-shrink: 0; box-shadow: 0 4px 14px rgba(14,157,175,0.3); }
    ...
    .tab-btn { display: flex; align-items: center; gap: 8px; padding: 10px 16px; border: none; background: none; border-radius: 10px; cursor: pointer; font-size: 13px; font-weight: 600; color: var(--c-muted); transition: all 0.2s; white-space: nowrap; }
    .tab-btn .tc { background: var(--c-gray-200); color: var(--c-muted); padding: 2px 8px; border-radius: 10px; font-size: 11px; font-weight: 700; }
    .tab-btn:hover { background: var(--c-gray-100); color: var(--c-text); }
    .tab-btn.active { background: var(--c-teal); color: white; box-shadow: 0 3px 10px rgba(14,157,175,0.3); }
    .tab-btn.active .tc { background: rgba(255,255,255,0.25); color: white; }

    .stats-row { display: flex; gap: 12px; margin-bottom: 18px; flex-wrap: wrap; }
    .stat-card { background: white; padding: 14px 20px; border-radius: var(--r); box-shadow: var(--sh); border: 1px solid var(--c-gray-200); display: flex; flex-direction: column; gap: 4px; cursor: pointer; transition: all 0.2s; position: relative; }
    .stat-card:hover { transform: translateY(-2px); box-shadow: var(--sh-md); }
    .stat-card.active { border-color: var(--c-teal); box-shadow: 0 0 0 3px rgba(14,157,175,0.15); }
    .sc-value { font-size: 26px; font-weight: 800; }
    .sc-value.primary { color: var(--c-teal); }
    .sc-value.success { color: var(--c-green); }
    .sc-value.warning { color: var(--c-amber); }
    .sc-value.info    { color: var(--c-blue); }
    .sc-value.danger  { color: var(--c-red); }
    .sc-label { font-size: 12px; color: var(--c-muted); font-weight: 500; }
    .sc-indicator { position: absolute; top: 8px; right: 8px; color: var(--c-teal); }

    .filters-row { display: flex; gap: 10px; margin-bottom: 16px; align-items: center; flex-wrap: wrap; }
    .search-box { flex: 1; display: flex; align-items: center; gap: 8px; background: white; border: 1.5px solid var(--c-gray-200); border-radius: var(--r); padding: 0 12px; transition: border-color 0.2s; min-width: 200px; }
    .search-box:focus-within { border-color: var(--c-teal); }
    .sb-input { flex: 1; border: none; outline: none; padding: 10px 0; font-size: 13px; background: transparent; }
    .sb-clear { background: none; border: none; cursor: pointer; color: var(--c-muted); display: flex; align-items: center; }
    .sb-clear:hover { color: var(--c-red); }
    .filter-sel { padding: 10px 14px; border: 1.5px solid var(--c-gray-200); border-radius: var(--r); font-size: 13px; outline: none; background: white; cursor: pointer; color: var(--c-text); transition: border-color 0.2s; }
    .filter-sel:focus { border-color: var(--c-teal); }
    ...
    tbody tr { border-bottom: 1px solid var(--c-gray-100); transition: background 0.15s; }
    tbody tr.tr-hover:hover { background: var(--c-gray-100); }
    tbody td { padding: 13px 16px; font-size: 13px; vertical-align: middle; }

    .emp-cell { display: flex; align-items: center; gap: 10px; }
    .emp-cell strong { display: block; font-size: 13px; color: var(--c-text); }
    .emp-cell small { font-size: 11px; color: var(--c-muted); }
    ...
    .emp-info { flex: 1; }
    .matricule-tag { display: inline-flex; align-items: center; gap: 4px; background: var(--c-teal-lt); color: var(--c-teal); padding: 4px 10px; border-radius: 8px; font-size: 12px; font-weight: 600; font-family: monospace; }
    .dept-tag { background: var(--c-gray-100); color: var(--c-text); padding: 3px 10px; border-radius: 20px; font-size: 12px; }
    .societe-tag { background: #BEE3F8; color: #2A69AC; padding: 3px 10px; border-radius: 20px; font-size: 12px; font-weight: 600; }
    ...
    .action-btn { width: 32px; height: 32px; border: none; background: var(--c-gray-100); border-radius: 8px; cursor: pointer; display: flex; align-items: center; justify-content: center; color: var(--c-muted); transition: all 0.2s; }
    .action-btn:hover { background: var(--c-teal-lt); color: var(--c-teal); transform: scale(1.1); }
    ...
    .role-badge { padding: 3px 10px; border-radius: 20px; font-size: 11px; font-weight: 700; display: inline-block; }
    ...
    .fch-icon { width: 48px; height: 48px; border-radius: 13px; background: linear-gradient(135deg, var(--c-teal), var(--c-teal-dk)); display: flex; align-items: center; justify-content: center; color: white; flex-shrink: 0; }
    ...
    .section-label { display: flex; align-items: center; gap: 7px; font-size: 12px; font-weight: 700; color: var(--c-teal); background: var(--c-teal-lt); padding: 7px 12px; border-radius: 8px; margin: 18px 0 12px; border-left: 3px solid var(--c-teal); }
    .grid-2 { display: grid; grid-template-columns: 1fr 1fr; gap: 14px; }
    .form-group { display: flex; flex-direction: column; gap: 5px; }
    .form-group label { font-size: 11px; font-weight: 700; color: var(--c-muted); text-transform: uppercase; letter-spacing: 0.5px; }
    ...
    input, select, textarea { padding: 10px 14px; border: 1.5px solid var(--c-gray-200); border-radius: var(--r); font-size: 13px; outline: none; background: white; color: var(--c-text); transition: border-color 0.2s; width: 100%; }
    input:focus, select:focus, textarea:focus { border-color: var(--c-teal); box-shadow: 0 0 0 3px rgba(14,157,175,0.1); }
    input.error, select.error { border-color: var(--c-red); }

    .manager-field { margin-top: 4px; }
    .manager-list-select { border: 1.5px solid var(--c-gray-200); border-radius: var(--r); overflow: hidden; max-height: 270px; overflow-y: auto; }
    .mls-option { display: flex; align-items: center; gap: 12px; padding: 11px 14px; cursor: pointer; transition: background 0.15s; border-bottom: 1px solid var(--c-gray-100); }
    .mls-option:last-child { border-bottom: none; }
    .mls-option:hover { background: var(--c-gray-100); }
    .mls-option.selected { background: var(--c-teal-lt); border-left: 3px solid var(--c-teal); }
    ...
    .mls-info { flex: 1; }
    .mls-info strong { font-size: 13px; color: var(--c-text); display: block; margin-bottom: 2px; }
    .mls-info small { font-size: 11px; color: var(--c-muted); display: flex; align-items: center; gap: 5px; flex-wrap: wrap; }
    .mls-check { color: var(--c-teal); flex-shrink: 0; }
    .mls-empty { display: flex; align-items: center; gap: 8px; padding: 18px; color: var(--c-amber); font-size: 13px; font-weight: 600; background: var(--c-amber-lt); }

    .info-notice { display: flex; align-items: center; gap: 7px; background: var(--c-teal-lt); border-radius: var(--r); padding: 10px 14px; font-size: 12px; color: var(--c-teal); margin: 16px 0; }
    .info-notice strong { color: var(--c-teal-dk); }

    .form-alert { display: flex; align-items: center; gap: 8px; padding: 11px 14px; border-radius: var(--r); font-size: 13px; margin-bottom: 12px; }
    .form-alert.error   { background: var(--c-red-lt);   color: var(--c-red); }
    .form-alert.success { background: var(--c-green-lt); color: var(--c-green); }
    ...
    .help-card, .managers-card, .no-managers-card { background: white; border-radius: var(--r-lg); padding: 18px; box-shadow: var(--sh); border: 1px solid var(--c-gray-200); }
    .help-card h4, .managers-card h4 { font-size: 13px; font-weight: 700; color: var(--c-text); margin-bottom: 14px; display: flex; align-items: center; gap: 6px; }
    .hc-list { display: flex; flex-direction: column; gap: 11px; }
    .hc-item { display: flex; gap: 10px; align-items: flex-start; }
    .hc-icon { width: 32px; height: 32px; border-radius: 8px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
    .hc-icon.teal  { background: var(--c-teal-lt);  color: var(--c-teal); }
    .hc-icon.blue  { background: var(--c-blue-lt);  color: var(--c-blue); }
    .hc-icon.green { background: var(--c-green-lt); color: var(--c-green); }
    .hc-icon.amber { background: var(--c-amber-lt); color: var(--c-amber); }
    .hc-item strong { font-size: 12px; color: var(--c-text); display: block; }
    .hc-item p { font-size: 11px; color: var(--c-muted); margin: 0; }
    .mc-item { display: flex; align-items: center; gap: 8px; padding: 7px 0; border-bottom: 1px solid var(--c-gray-100); }
    .mc-item:last-child { border-bottom: none; }
    .mc-av { width: 28px; height: 28px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 10px; font-weight: 700; color: white; flex-shrink: 0; }
    .mc-item strong { font-size: 12px; color: var(--c-text); display: block; }
    .mc-item small  { font-size: 11px; color: var(--c-muted); }
    .no-managers-card { display: flex; gap: 10px; align-items: flex-start; background: var(--c-amber-lt); border: 1px solid #ecc94b; }
    .nm-icon { flex-shrink: 0; color: var(--c-amber); }
    ...
    .eh-info { flex: 1; }
    .eh-info h2 { font-size: 18px; font-weight: 700; color: var(--c-text); margin-bottom: 3px; }
    .eh-info p  { font-size: 13px; color: var(--c-muted); margin-bottom: 6px; }
    ...
    .ec-header-left h3 { font-size: 15px; font-weight: 700; color: var(--c-text); margin: 0; }
    .ec-icon { width: 32px; height: 32px; border-radius: 8px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
    .ec-icon.teal  { background: var(--c-teal-lt);  color: var(--c-teal); }
    .ec-icon.amber { background: var(--c-amber-lt); color: var(--c-amber); }
    .ec-icon.red   { background: var(--c-red-lt);   color: var(--c-red); }
    ...
    .btn { padding: 10px 18px; border-radius: var(--r); font-size: 13px; font-weight: 600; cursor: pointer; border: none; display: inline-flex; align-items: center; gap: 6px; transition: all 0.2s; }
    .btn-primary { background: linear-gradient(135deg, var(--c-teal), var(--c-teal-dk)); color: white; box-shadow: 0 3px 10px rgba(14,157,175,0.3); }
    .btn-primary:hover:not(:disabled) { transform: translateY(-1px); box-shadow: 0 5px 14px rgba(14,157,175,0.4); }
    .btn-outline { background: none; border: 1.5px solid var(--c-gray-200); color: var(--c-muted); }
    .btn-outline:hover { border-color: var(--c-teal); color: var(--c-teal); }
    .btn:disabled { opacity: 0.55; cursor: not-allowed; }

    .toast { position: fixed; bottom: 24px; right: 24px; padding: 13px 18px; border-radius: var(--r); font-size: 13px; font-weight: 600; transform: translateY(80px); opacity: 0; transition: all 0.3s ease; z-index: 2000; box-shadow: 0 8px 24px rgba(0,0,0,0.12); display: flex; align-items: center; gap: 8px; }
    .toast.show { transform: translateY(0); opacity: 1; }
    .toast.toast--success { background: var(--c-green-lt); color: var(--c-green); }
    .toast.toast--error   { background: var(--c-red-lt);   color: var(--c-red); }
...
export class EmployesComponent implements OnInit {
  ...
  filterRole    = signal('');
  filterDept    = signal('');
  filterSociete = signal(''); // ✅ FILTRE SOCIÉTÉ
  ...
  setTab(tab: Tab): void {
    ...
    this.createError.set(''); this.createSuccess.set('');
    this.editError.set('');   this.editSuccess.set('');
  }

  onRoleChange(): void {
    if (this.createForm.get('role')?.value === 'ADMIN')
      this.createForm.get('managerId')?.setValue(null);
  }
  ...
  getManagersList(): any[] {
    return this.employes().filter(e =>
      ['MANAGER','RH','ADMIN'].includes((e.role ?? '').toUpperCase()));
  }
  ...
  onCreate(): void {
    ...
    this.http.post(`${this.API}/rh/employes`, payload).subscribe({
      next: () => {
        this.createLoading.set(false);
        this.createSuccess.set('Compte créé ! Email de bienvenue envoyé.');
        this.resetCreate();
        this.loadData();
        setTimeout(() => { this.createSuccess.set(''); this.setTab('liste'); }, 2000);
      },
      error: (err) => {
        this.createLoading.set(false);
  ...
  ouvrirModifier(e: any): void {
    ...
    this.editForm.patchValue({
      ...
      salaireBase: e.salaireBase ?? null,
      matricule: e.matricule ?? '', dateEmbauche: e.dateEmbauche ?? '',
      managerId: e.managerId ?? null, cin: e.cin ?? '',
      telephone: e.telephone ?? '', adresse: e.adresse ?? '',
      ville: e.ville ?? '', codePostal: e.codePostal ?? '',
      nationalite: e.nationalite ?? '',
      situationFamiliale: e.situationFamiliale ?? '',
      contactUrgenceNom: e.contactUrgenceNom ?? '',
      contactUrgenceTelephone: e.contactUrgenceTelephone ?? '',
      contactUrgenceRelation: e.contactUrgenceRelation ?? '',
      societe: e.societe ?? ''  // ✅
    });
  ...
  onSaveEdit(): void {
    ...
    this.http.put(`${this.API}/rh/employes/${emp.id}`, payload).subscribe({
      next: (data: any) => {
        ...
        this.editSuccess.set('Modifications enregistrées avec succès !');
        this.employes.update(l => l.map(e => e.id === emp.id ? data : e));
        this.selectedEmploye.set(data);
  ...
  // ✅ getFiltered avec filtre société
  getFiltered(): any[] {
    return this.employes().filter(e => {
      const t  = this.search().toLowerCase();
      const r  = (this.filterRole() ?? '').toUpperCase();
      const d  = this.filterDept();
      const s  = this.filterSociete();
      const ms = !t || ['nom','prenom','email','matricule','cin','poste']
        .some(k => (e[k] ?? '').toLowerCase().includes(t));
      return ms
        && (!r || (e.role ?? '').toUpperCase() === r)
        && (!d || e.departement === d)
        && (!s || e.societe === s); // ✅
    });
  }
  ...
  getDepts(): string[] {
    return [...new Set(this.employes().map(e => e.departement).filter(Boolean))].sort() as string[];
  }

  // ✅ getSocietes
  getSocietes(): string[] {
    return [...new Set(this.employes().map(e => e.societe).filter(Boolean))].sort() as string[];
  }
  ...
  getRoleLabel(role: string): string {
    const map: Record<string,string> = {
      EMPLOYE: 'Employé', MANAGER: 'Manager', RH: 'RH', ADMIN: 'Admin'
    };
